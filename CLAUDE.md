...
**Pattern F — Graphics-heavy warning（用戶要 compress timeline 時）：**

當用戶提出想再壓縮 timeline，**先問 graphics 量**，唔好直接照壓：

> 「壓縮 timeline 可以，但要睇你嘅 graphics 量。Corporate video 如果 graphics 多（e.g. collage animation、大量 motion graphics，好似之前 CLP Chat with Joe 嗰款），後期通常壓縮唔到；要壓嘅話成品就要相對 simple。你呢條片 graphics 量點？」

根據答案：
- **Graphics 少** → OK，提議具體 compression（e.g. 1st Cut 5 wd → 4 wd）
- **Graphics 多** → 提醒「唯有跟返標準 timeline，squeeze 唔到。如果 deadline 真係郁唔到就要 simplify graphics 嘅 scope」
